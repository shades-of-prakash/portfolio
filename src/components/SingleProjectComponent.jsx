...
const SingleProjectComponent = ({ project, setter }) => {
	...
	return (
		...
		>
			<div
				className="border-1 border-green-600/50 rounded-xl test md:h-[660px] h-full md:w-[1200px] w-screen overflow-scroll flex items-center flex-col"
				ref={modalContentRef} // Assign the ref to the modal content div
				onClick={(e) => e.stopPropagation()} // Stop propagation for clicks INSIDE the modal
			>
				...
				<div className="w-full bg-black overflow-scroll p-1 md:p-5">
							...
							<div className="max-w-fit flex-wrap h-auto mt-4 text-nowrap flex items-center gap-1">
								{projectTechStack.map((stack, index) => {
									...
									return (
										<div
											key={index}
											className="bg-neutral-950 border border-white/[0.14] text-white/80 flex px-4 py-1.5 rounded-full items-center gap-2 text-sm"
										>
					...
					{projectImages.slice(1).map((img, index) => {
						return (
							<div
								key={index}
								className="w-full h-50 md:h-full pt-2 px-2 md:p-0"
							>
